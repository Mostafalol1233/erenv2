...
function Logo() {
  return <Link className="eren-logo" href="/" aria-label="إيرين ستور - الصفحة الرئيسية"><img className="eren-logo-image" src="/images/eren-logo-cinematic-v3.svg" alt="إيرين" /><span><strong>إيرين</strong><small>متجر اللاعبين</small></span></Link>
}
...
function SectionTitle({ label, title, id }: { label: string; title: string; id?: string }) {
  return <div className="xbv-section-title" id={id}><div><span className="xbv-section-label">{label}</span><h2>{title}</h2></div><Link href="#all-products" className="xbv-view-all">عرض الكل <ArrowLeft /></Link></div>
}
...
export default function ErenStoreLanding() {
  ...
  return <main className="xbv-store-shell">
    ...
    <section id="home" className="xbv-hero"><div className="xbv-hero-mosaic">{mosaicGames.map((game, index) => <img key={game.slug} className={`xbv-mosaic-image mosaic-${index + 1}`} src={game.image} alt="" />)}</div><div className="xbv-hero-overlay" /><div className="container xbv-hero-content"><span className="xbv-hero-kicker">متجر إيرين للاعبين</span><h1>كل شيء يحتاجه<br /><em>اللاعب.</em></h1><p>شحن سريع، بطاقات رقمية، وباقات واضحة لكل ألعابك المفضلة.</p><div className="xbv-hero-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ابحث عن لعبة أو بطاقة رقمية" aria-label="البحث عن لعبة أو بطاقة رقمية" /><button onClick={() => scrollTo("all-products")} aria-label="ابدأ البحث"><ArrowLeft /></button></div><div className="xbv-trending-searches"><span>عمليات البحث الرائجة</span>{featuredGames.slice(0, 5).map((game) => <button key={game.slug} onClick={() => { setQuery(game.name); scrollTo("all-products") }}>{game.name}</button>)}</div></div></section>

    <section className="container xbv-promos"><div className="xbv-rewards-card"><div className="xbv-reward-top"><span className="xbv-reward-icon" aria-hidden="true" /><div><span>مرحباً أيها اللاعب</span><strong>مكافآت إيرين</strong></div><b>جديد</b></div><div className="xbv-progress-line"><span>البرونزي</span><strong>0 XP</strong><span>تقدّم البداية</span></div><div className="xbv-progress"><span /></div><div className="xbv-reward-foot"><small>اجمع نقاطاً مع كل طلب</small><a href="#reviews">اعرف المزيد</a></div></div><Link href="/games/fortnite" className="xbv-promo-banner"><img src="/images/revamp/fortnite.jpg" alt="Fortnite" /><span className="xbv-promo-shade" /><div><span>عرض الأسبوع</span><h2>استعد للجولة<br /><em>واشحن الآن.</em></h2><strong>اكتشف الباقات <ArrowLeft /></strong></div></Link></section>
